// protocol.cpp - Protocol Implementation (CRC16)
...
bool verify_control_frame(const ControlFrame* frame) {
    ...
    // Check version
    if (frame->version != ControlConfig::VERSION) {
        return false;
    }

    // Calculate CRC over all bytes except the last 2 (CRC field)
    uint16_t calc_crc = crc16_ccitt((const uint8_t*)frame, sizeof(ControlFrame) - 2);

    return calc_crc == frame->crc16;
}
